...
function pvm() {
    ...
    if [[ $1 == 'use' ]]; then
        if [[ $2 == 'system' ]]; then
            ...
            if [[ -a $composerjson_path ]]; then
                pvm_version=$(cat $composerjson_path | jq '.require.php' | grep -Eo '[0-9](.[0-9])?' | head -n 1)
                log "use v$pvm_version from composer.json"
                
                if [[ "$(echo "$pvm_version < 7.3" | bc -l)" -eq 1 ]]; then
                    echo "fallback to 7.3 as yeah my preference"
                    pvm_version="7.3"
                fi
