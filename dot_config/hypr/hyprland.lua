...
-- ── OBS (Recording) ───────────────────────────
local obs = "obs-cmd"
hl.bind("SUPER + F8", hl.dsp.exec_cmd(obs .. ' recording toggle && notify-send "OBS" "Recording toggled"'))
hl.bind("SUPER + F9", hl.dsp.exec_cmd(obs .. ' recording toggle-pause && notify-send "OBS" "Recording paused/resumed"'))
hl.bind("SUPER + F10", hl.dsp.exec_cmd(obs .. ' replay-buffer save && notify-send "OBS" "Replay saved!"'))
hl.bind("SUPER + F11", hl.dsp.exec_cmd(obs .. ' scene switch "Screen" && notify-send "OBS" "Scene: DevVlog"'))
hl.bind("SUPER + SHIFT + F8", hl.dsp.exec_cmd(obs .. ' recording create-chapter && notify-send "OBS" "Chapter marked"'))
...
local function assign_workspaces()
  ...
  if has_dp2 then
    ...
    hl.dispatch(hl.dsp.workspace.move({ workspace = 1, monitor = "DP-2" }))
  end
